...
st.set_page_config(page_title="Deal Sourcing Engine", page_icon="⚙️", layout="wide", initial_sidebar_state="expanded")
inject_css()
...
if top is not None:
    ...
    with t2:
        ...
        <div style="background:var(--paper2);padding:20px 22px;border:1px solid rgba(16,14,12,.08)">
            ...
            <div style="font-family:var(--serif);font-size:36px;font-weight:500;color:var(--gold)">{top.get("Score",0):.0f}</div>
        </div>""", unsafe_allow_html=True)
    with t3:
        st.markdown(f"""
        <div style="background:var(--paper2);padding:20px 22px;border:1px solid rgba(16,14,12,.08)">
            ...
            <div style="font-family:var(--serif);font-size:36px;font-weight:500;color:var(--ink)">{top.get("EV/EBITDA",0):.1f}×</div>
        </div>""", unsafe_allow_html=True)
    with t4:
        st.markdown(f"""
        <div style="background:var(--paper2);padding:20px 22px;border:1px solid rgba(16,14,12,.08)">
            ...
            <div style="font-family:var(--serif);font-size:36px;font-weight:500;color:var(--green)">{top.get("EBITDA Margin %",0):.1f}%</div>
        </div>""", unsafe_allow_html=True)
...
for col, pg, icon, title, desc in [
    ...
    with col:
        ...
        st.page_link(pg, label=f"→ {title}", use_container_width=True)
